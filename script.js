...
// ==========================================================================
// ON-DEMAND ADMIN STUDIO LAZY LOADER
// (Loads admin.js and admin.css only when requested to keep visitor payload ultra-fast)
// ==========================================================================
let isAdminLoading = false;
let isAdminLoaded = false;

function loadAdminStudio(callback) {
  if (isAdminLoaded) {
    if (typeof callback === "function") callback();
    return;
  }
  if (isAdminLoading) {
    window.addEventListener("admin_studio_ready", () => {
      if (typeof callback === "function") callback();
    }, { once: true });
    return;
  }
  ...
  // 2. Inject admin.js
  if (!document.getElementById("admin-js-script")) {
    const script = document.createElement("script");
    script.id = "admin-js-script";
    script.src = "admin/admin.js";
    script.onload = () => {
      isAdminLoaded = true;
      isAdminLoading = false;
      window.dispatchEvent(new CustomEvent("admin_studio_ready"));
      if (typeof callback === "function") callback();
    };
    script.onerror = () => {
      isAdminLoading = false;
      console.error("Failed to load admin studio bundle.");
    };
    document.body.appendChild(script);
  }
}

// Attach Admin Triggers (Button, Shortcut, Triple-Click, URL)
function initAdminTriggers() {
  // 1. Admin Trigger Button in Footer
  const triggerBtn = document.getElementById("adminTriggerBtn");
  if (triggerBtn) {
    triggerBtn.onclick = (e) => {
      e.preventDefault();
      loadAdminStudio(() => {
        if (typeof window.openAdminPinModal === "function") window.openAdminPinModal();
      });
    };
  }
  ...
  // 3. URL Parameter Trigger (?admin=1 or #admin)
...
// 4. Keyboard Shortcut (Ctrl + Shift + A / Cmd + Shift + A)
window.addEventListener("keydown", (e) => {
  if ((e.ctrlKey || e.metaKey) && e.shiftKey && (e.key === "A" || e.key === "a")) {
    e.preventDefault();
    loadAdminStudio(() => {
      if (typeof window.toggleAdminStudio === "function") {
        window.toggleAdminStudio();
      } else if (typeof window.openAdminPinModal === "function") {
